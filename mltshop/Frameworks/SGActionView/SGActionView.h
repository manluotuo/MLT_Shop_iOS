...
typedef void(^SGPickerMenuActionHandler)(NSDate* theDate);
typedef void(^SGMultiMenuActionHandler)(NSMutableArray* selectedItems);
 ...
 *
 *	@param 	title       标题
 *	@param 	pickerMode 	元素标题
 *	@param 	handler 	回调，元素index从 1 开始，0 为取消。
 */
+ (void)showPickerMenuWithTitle:(NSString *)title
                     pickerMode:(UIDatePickerMode)pickerMode
                           date:(NSDate *)theDate
               selectedHandle:(SGPickerMenuActionHandler)handler;

/**
 *  多选弹出菜单, 返回NSMutableNSArray
 *
 *  @param title      标题
 *  @param itemTitles 选择列表
 *  @param handler    返回 NSArray(NSNumber,,)
 */
+ (void)showMultiWithTitle:(NSString *)title
                itemTitles:(NSArray *)itemTitles
            selectedHandle:(SGMultiMenuActionHandler)handler;
