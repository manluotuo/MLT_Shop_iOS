...
+ (void)showPickerMenuWithTitle:(NSString *)title
    ...
    SGPickerMenu *menu = [[SGPickerMenu alloc]initWithTitle:title
                                                 pickerMode:pickerMode
                                                       date:theDate];
    [menu triggerSelectedAction:^(NSDate *theDate) {
        [[SGActionView sharedActionView] dismissMenu:menu Animated:YES];
        if (handler) {
            handler(theDate);
        }
...
+ (void)showMultiWithTitle:(NSString *)title
    ...
    SGMultiMenu *menu = [[SGMultiMenu alloc] initWithTitle:title
                                                itemTitles:itemTitles];
    [menu triggerSelectedAction:^(NSMutableArray * result) {
        [[SGActionView sharedActionView] dismissMenu:menu Animated:YES];
        if (handler) {
            handler(result);
        }
