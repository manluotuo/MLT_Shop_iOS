...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.subItems.count > 0) {
        ...
        NSString *theTitle = self.items[indexPath.row];
        CGSize size = [theTitle sizeWithWidth:H_250 andFont:FONT_16];
        if (size.height > H_30) {
            return size.height+H_15;
        }else{
            return 44;
        }
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.subItems.count > indexPath.row) {
        ...
        if (![subTitle isEqual:[NSNull null]]) {
            ...
            [cell.imageView setImage:[UIImage imageNamed:self.subItems[indexPath.row]]];
        }
    }else{
        [cell.imageView setFrame:CGRectZero];
    }
