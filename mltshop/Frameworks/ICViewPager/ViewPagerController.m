...
@property (nonatomic) NSNumber *tabViewWidth;
@property (nonatomic) NSNumber *tabOffset;
@property (nonatomic) NSNumber *tabWidth;
@property (nonatomic) NSArray  *tabWidthArray;
...
@synthesize tabViewWidth = _tabViewWidth;
@synthesize tabOffset = _tabOffset;
@synthesize tabWidth = _tabWidth;
@synthesize tabWidthArray = _tabWidthArray;
...
- (void)layoutSubviews {
    ...
    frame.size.width =  [self.tabViewWidth floatValue];
...
- (void)setTabViewWidth:(NSNumber *)tabViewWidth {
    
//    if ([tabViewWidth floatValue] > 320.0)
//        tabViewWidth = [NSNumber numberWithFloat:320.0];
    
    _tabViewWidth = tabViewWidth;
}
...
- (NSNumber *)tabViewWidth {
    
    if (!_tabViewWidth) {
        CGFloat value = kTabViewWidth;
        if ([self.delegate respondsToSelector:@selector(viewPager:valueForOption:withDefault:)])
            value = [self.delegate viewPager:self valueForOption:ViewPagerOptionTabViewWidth withDefault:value];
        self.tabViewWidth = [NSNumber numberWithFloat:value];
    }
...
- (NSArray *)tabWidthArray{
    if (!_tabWidthArray) {
        NSArray* value = [[NSArray alloc]init];
        if ([self.delegate respondsToSelector:@selector(viewPager:valueForOption:withDefault:)])
            value = [self.delegate viewPager:self valueArrayForOption:ViewPagerOptionTabWidthArray];
        self.tabWidthArray = value;
    }
    return _tabWidthArray;
}
...
- (void)setNeedsReloadOptions {
    ...
    self.tabWidthArray = [NSArray arrayWithArray:[self.delegate viewPager:self valueArrayForOption:ViewPagerOptionTabWidthArray]];
    ...
    for (NSUInteger i = 0; i < self.tabCount; i++) {
        ...
        CGFloat theTabWidth =  [[self.tabWidthArray objectAtIndex:i] floatValue];
        if (theTabWidth > 0) {
            frame.size.width = theTabWidth;
        }else{
...
- (CGFloat)valueForOption:(ViewPagerOption)option {
    ...
    switch (option) {
        ...
        case ViewPagerOptionTabViewWidth:
            return [[self tabViewWidth] floatValue];
...
- (void)defaultSetup {
    ...
    if (!self.tabsView) {
        ...
        self.tabsView = [[UIScrollView alloc] initWithFrame:CGRectMake(0.0, 0.0, [self.tabViewWidth floatValue], [self.tabHeight floatValue])];
