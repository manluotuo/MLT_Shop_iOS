	...
	if (numberOfLevels > 0) {
		...
		for (int i = 0; i < numberOfLevels; i++) {
			...
			if (i < [self.tableViews count]) {
                ...
                if (i > 0) {
                    UIView *columnSepView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, 1, 5000)];
                    columnSepView.backgroundColor = GRAYLIGHTCOLOR;
                    [tableView addSubview:columnSepView];
                    [tableView bringSubviewToFront:columnSepView];
...
- (NSArray *)sectionIndexTitlesForTableView:(UITableView *)tableView
{
    ...
    if ([self.dataSource respondsToSelector:@selector(multiTablesView:sectionIndexTitlesForTableViewAtLevel:)]) {
        array = [self.dataSource multiTablesView:self sectionIndexTitlesForTableViewAtLevel:[self indexOfTableView:tableView]];
    }
    return array;
}
...
- (void)popCurrentTableViewAnimated:(BOOL)animated {
	if (self.currentTableViewIndex > 0) {
		...
		[UIView animateWithDuration:animated?ANI_DUR:0.0
							  delay:0.0
							options:UIViewAnimationCurveEaseInOut
						 animations:^{
